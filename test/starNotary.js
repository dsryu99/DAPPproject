...
contract('StarNotary', async (accs) => {
  ...
  it('can Create a Star', async() => {
    ...
    assert.deepEqual(await instance.lookUpTokenIdToStarInfo(tokenId), 'Awesome Star!')
  });
...
// 3) Stars Tokens can be transferred from one address to another.
  ...
  it('Token name and symbol are added', async() => {
    let tokenId = 6;
    await instance.createStar('Awesome Star!', tokenId, {from: accounts[0]})
    assert.deepEqual(await instance.lookUpTokenIdToStarInfo(tokenId), 'Awesome Star!')
  });
  ...
  it('Star Tokens can be transferred from one address to another', async() => {
    ...
    await instance.transferAStar(user2, starId1, {from: user1})
    assert.equal(await instance.ownerOf.call(starId1), user2);
  });